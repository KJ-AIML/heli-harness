...
Heli-Harness is the workspace governance source of truth. Kimi Code CLI must treat `.heli-harness/HARNESS.md` as authoritative for harness protocol, and this file only translates that protocol into Kimi-facing startup behavior.
...
6. Read repo-local `AGENTS.md`, `.kimi-code/AGENTS.md`, `README*`, package/build/test files, and relevant docs.
...
Kimi hooks are configured via `config.toml` `[[hooks]]` entries (or AgentHooks). Before treating guardrails as live, confirm the Heli PreToolUse hook is registered and running. If hooks are missing, every rule in this file is advisory only — say so explicitly before S2/S3 work.
...
- This Kimi adapter is instruction + hooks-config based.
- Hook scripts are Claude-style JSON deny on PreToolUse and are smoke-tested synthetically.
- Live Kimi CLI denial proof is not yet shipped (status: `enforced`, not `enforced`).
- Kimi also reads project `AGENTS.md` / `.kimi-code/AGENTS.md` for instructions.

Kimi-specific behavior belongs here. Core harness files must remain tool-neutral.
