...
- For parallel agents: use `heli status` to inspect linked resource authority. The `concurrent-upgrade` skill is only for embedded compatibility workspaces that still use legacy shared task state.
