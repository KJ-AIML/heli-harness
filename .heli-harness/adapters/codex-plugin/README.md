...
This plugin follows the Codex plugin layout: `.codex-plugin/plugin.json`, root-level `skills/` (full canonical Heli skill library, kept in sync via `npm run sync:plugin-skills`), and root-level `hooks/hooks.json`.
