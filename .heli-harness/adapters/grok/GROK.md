...
Heli-Harness is the workspace governance source of truth. Grok Build must treat `.heli-harness/HARNESS.md` as authoritative for harness protocol, and this file only translates that protocol into Grok-facing startup behavior.
...
Before treating any guardrail here as enforced rather than advisory, check whether Heli hooks actually loaded this session. Grok is Claude Code–compatible and also loads native Grok hooks/plugins. Look for injected context starting with "Heli-Harness plugin context:" when SessionStart injection is available; if hooks were never trusted (`/hooks-trust` or `--trust`) or the plugin was not loaded, denials will not fire and every rule in this file is advisory only. Say so explicitly to the user before doing S2/S3 work.
...
- This Grok adapter is instruction + native hook/plugin based.
- Grok auto-reads `AGENTS.md` / `CLAUDE.md` and Claude Code plugins/hooks with zero extra config.
- Native Heli Grok plugin hooks are smoke-tested synthetically; live Grok CLI denial proof is not yet shipped (status: `enforced`, not `enforced`).
- Grok project hooks require `/hooks-trust` or `--trust` before they run.
- Plugin skill surface is heli-governance + heli-target + heli-install only.

Grok-specific behavior belongs here. Core harness files must remain tool-neutral.
