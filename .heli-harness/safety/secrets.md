...
- Pi/AXGA v0.5.6 guard obvious secret paths such as `.env*`, key files, credential files, registry configs, and SSH private-key style paths where compatible `tool_call` hooks expose paths.
- Pi/AXGA v0.5.6 guard obvious secret-like write content such as common API key prefixes and private-key blocks where compatible `tool_call` hooks expose content.
