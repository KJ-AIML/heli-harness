...
Use the latest stable tag (v0.5.3). Do not install globally. Treat the current
...
`command-rules.json` is consumed by the Pi/AXGA runtime guard where compatible `tool_call` hooks are available. These rules drive command blocking/confirmation for configured bash patterns. This is not a sandbox; it is an adapter-level guard that depends on host hook support. v0.5.4 will harden command classification beyond simple pattern matching.
...
- **Status: supported** - verified with AXGA and Pi loading v0.5.3.
