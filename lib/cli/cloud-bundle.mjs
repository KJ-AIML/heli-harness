...
} from "../concurrency/portable-targets.mjs";
import { heliDir as workspaceHeliDir } from "../concurrency/paths.mjs";
...
function bundleWorkspaceIndex(workspaceRoot, files) {
	try {
		const parsed = JSON.parse(files["workspace/index.json"] || "");
		if (parsed && typeof parsed === "object") return parsed;
	} catch {
		if (files["workspace/index.json"] != null) {
			throw new Error("Portable bundle workspace/index.json is not valid JSON.");
		}
	}
	return readWorkspaceIndex(workspaceRoot);
}

function rewriteTaskFiles(workspaceRoot, files, transform) {
	const next = { ...files };
	const workspaceIndex = bundleWorkspaceIndex(workspaceRoot, files);
	for (const rel of taskEntries(files)) {
		let task;
		try {
			task = JSON.parse(files[rel]);
		} catch {
			throw new Error(`Portable bundle task file is not valid JSON: ${rel}`);
		}
		if (!task || typeof task !== "object" || !task.target || typeof task.target !== "object") continue;
		const result = transform(workspaceRoot, task.target, workspaceIndex);
		const target = result?.target || result;
		next[rel] = `${JSON.stringify({ ...task, target }, null, 2)}\n`;
	}
	return next;
}

/** Normalize task targets for the portable bundle without mutating local files. */
export function normalizeTaskFilesForBundle(workspaceRoot, files) {
	return rewriteTaskFiles(workspaceRoot, files, (root, target, index) =>
		 sanitizeTaskTargetForBundle(root, target, index),
	);
}

/** Restore task targets against the current workspace root before writing them. */
export function restoreTaskFilesForWorkspace(workspaceRoot, files) {
	return rewriteTaskFiles(workspaceRoot, files, (root, target, index) =>
		 migrateTaskTargetForRestore(root, target, index),
	);
}
...
export function collectBundleFiles(workspaceRoot) {
	...
	return normalizeTaskFilesForBundle(workspaceRoot, files);
}
