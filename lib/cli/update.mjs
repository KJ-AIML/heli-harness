import { existsSync, mkdtempSync, cpSync, rmSync, statSync, mkdirSync, writeFileSync, readFileSync } from "node:fs";
...
/**
 * Merge newly shipped safety defaults without overwriting a user's local
 * rules. Matching entries by id makes update drift deterministic and keeps
 * explicit local customizations intact.
 */
function mergeSafetyDefault(sourceHarnessDir, targetHarnessDir, fileName, collectionKey) {
	const sourcePath = join(sourceHarnessDir, "safety", fileName);
	const targetPath = join(targetHarnessDir, "safety", fileName);
	const source = readJsonFile(sourcePath);
	if (!source || typeof source !== "object") return { fileName, changed: false, reason: "source-missing" };
	const local = readJsonFile(targetPath);
	if (!local || typeof local !== "object") {
		mkdirSync(dirname(targetPath), { recursive: true });
		writeFileSync(targetPath, `${JSON.stringify(source, null, 2)}\n`, "utf8");
		return { fileName, changed: true, reason: "created-from-defaults" };
	}
	const sourceEntries = Array.isArray(source[collectionKey]) ? source[collectionKey] : [];
	const localEntries = Array.isArray(local[collectionKey]) ? local[collectionKey] : [];
	const ids = new Set(localEntries.map((entry) => entry?.id).filter(Boolean));
	const additions = sourceEntries.filter((entry) => entry?.id && !ids.has(entry.id));
	if (!additions.length && local.defaultsVersion === source.defaultsVersion) return { fileName, changed: false, reason: "current" };
	const merged = {
		...local,
		[collectionKey]: [...localEntries, ...additions],
		defaultsVersion: source.defaultsVersion || local.defaultsVersion || null,
	};
	writeFileSync(targetPath, `${JSON.stringify(merged, null, 2)}\n`, "utf8");
	return { fileName, changed: additions.length > 0 || local.defaultsVersion !== merged.defaultsVersion, reason: additions.length ? "merged-new-defaults" : "marked-current" };
}

function reconcileSafetyDefaults(sourceHarnessDir, targetHarnessDir) {
	return [
		mergeSafetyDefault(sourceHarnessDir, targetHarnessDir, "command-rules.json", "rules"),
		mergeSafetyDefault(sourceHarnessDir, targetHarnessDir, "expensive-actions.json", "actions"),
	];
}
...
export function update(sourceHarnessDir, parentDir, { resetState = false } = {}) {
	...
	// Safety overlays are user-owned, but newly shipped defaults must not drift
	// away forever. Merge by stable id and preserve all local entries/fields.
	const safetyMigration = reconcileSafetyDefaults(sourceHarnessDir, target);

	return { target, parentFull, preserveDirs, safetyMigration };
}
...
export function runUpdate(packageRoot, args) {
	...
	if (result.safetyMigration?.some((item) => item.changed)) {
		console.log("Safety defaults merged by rule/action id; local overlay entries were preserved.");
	}
	console.log(
