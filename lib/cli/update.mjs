...
export function update(sourceHarnessDir, parentDir, { resetState = false } = {}) {
	...
	// A checkout where the package root and the workspace are the same
	// directory (e.g. running /heli-update from inside a heli-harness
	// clone) makes source and target resolve identically; fs.cpSync would
	// otherwise fail below with a cryptic ERR_FS_CP_EINVAL crash.
	...
	const preserveDirs = resetState
		? PRESERVE_DIRS
		: [...PRESERVE_DIRS, "state", ...CONCURRENT_STATE_DIRS];
...
export function runUpdate(packageRoot, args) {
	...
	if (!resetState) {
		...
		console.log("Use --reset-state to replace state/ and concurrent task/session data from the repo checkout.");
	} else {
		console.log("Preserved local overlays: profiles/, workspace/, policies/, safety/.");
		console.log("state/ and concurrent task/session/bindings/locks were replaced from the repo checkout (--reset-state).");
	}
