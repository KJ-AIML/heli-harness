...
import { readJson, writeJsonAtomic } from "../concurrency/fs-atomic.mjs";
import {
	collectBundleFiles,
	contentSha256,
	packBundle,
...
function requireWorkspace(args) {
	...
	const root =
		...
		(existsSync(join(start, ".heli-harness")) ||
		existsSync(join(start, ".heli", "workspace.json"))
			? start
			: null);
	if (!root) throw new Error(`No Heli workspace found at or above: ${start}`);
	return root;
}

function syncStatePath(workspaceRoot) {
	return join(pathsFor(workspaceRoot).stateDir, "sync.json");
}
