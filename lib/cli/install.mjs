 ...
 */
export function install(sourceHarnessDir, parentDir, { force = false } = {}) {
	...
	if (existsSync(target) && !force) {
		throw new Error(
			`A Heli workspace already exists at ${target}.\n` +
				"Install would delete its profiles, policies, tasks, and sessions.\n" +
				"Use `heli update` to upgrade in place, or `heli install --force` to wipe and reinstall.",
		);
	}
...
export function runInstall(packageRoot, args) {
	...
	const [parentArg] = args.filter((arg) => arg !== "--force");
	const parentDir = parentArg || process.cwd();
	const sourceHarnessDir = join(packageRoot, ".heli-harness");
	if (!existsSync(sourceHarnessDir)) {
		throw new Error(`Source harness not found: ${sourceHarnessDir}`);
	}
	const result = install(sourceHarnessDir, parentDir, { force });
...
Clean install seed:
  - workspace mode: concurrent (default)
  - idle current-task projection (no selected target)
  - strict YOLO (no yolo.json)
  - empty concurrent stores (tasks/sessions/bindings/locks created on demand)
  - zero tasks => single-agent bootstrap writes allowed; create/claim tasks before multi-agent work
...
Next steps:
  ...
  3. For multi-agent work: heli task create <id> --work-item <key> --repo <name> && heli task claim <id> --mode write (export HELI_SESSION_ID)
  4. Start agents from ${result.parentFull}
  5. Use this first-run prompt:

Start from this parent workspace. Read HARNESS.md and heli status (mode should be concurrent). Identify the target repo, read its profile if present, then inspect repo-local docs. For parallel agents, create/claim a task before editing. Task: <describe task>.`);
}
