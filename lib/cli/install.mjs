...
export function install(sourceHarnessDir, parentDir) {
	...
	// directory (e.g. running install from inside a heli-harness clone)
	// makes source and target resolve identically; fs.cpSync would
	...
	for (const check of ["HARNESS.md", "manifest.json", "skills", "adapters"]) {
...
export function runInstall(packageRoot, args) {
	...
	console.log(`
Heli-Harness installed to:
  ${result.target}

Next steps:
  1. Add repo profiles under ${result.target}/profiles
  2. Start agents from ${result.parentFull}
