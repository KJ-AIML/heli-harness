...
import { findWorkspaceRoot, canonicalizePath, pathsFor } from "../concurrency/paths.mjs";
import {
	readProjectBinding,
	readWorkspaceLock,
	resolveExecutionIdentity,
} from "../concurrency/project-binding.mjs";
import { projectTaskWorktree, readWorkspaceIndex } from "../concurrency/portable-targets.mjs";
...
export function status(cwd) {
	...
	if (concurrent) {
		...
		const taskSummaries = active.map((t) => {
			...
			if (!layout.linked && lease && !lease.invalid && !isLeaseExpired(lease)) writeLeases++;
			return proj;
		});
		return {
			...base,
...
export function runStatus(args) {
	...
	if (result.layout === "linked") console.log("Authority model: resource-scoped");
	else console.log(`Workspace mode: ${result.mode}`);
	...
	if (result.mode === "concurrent") {
		...
		console.log(result.layout === "linked" ? "Current work record: optional; see work records above" : "Current task: see active tasks above (concurrent mode)");
		return;
	}
