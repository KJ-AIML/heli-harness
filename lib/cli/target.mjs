...
import { findWorkspaceRoot, resolveWorktreeRoot, pathsFor as workspacePathsFor } from "../concurrency/paths.mjs";
import { isConcurrentMode } from "../concurrency/schema.mjs";
import { resolveExecutionContext } from "../concurrency/resolve.mjs";
import { setTaskTarget, readTask, listTasks } from "../concurrency/task.mjs";
...
function paths(cwd) {
	...
	const layout = workspacePathsFor(root);
	return {
		workspaceRoot: root,
...
export function setTarget(cwd, selector, { confirm = false } = {}) {
	...
	const index = existsSync(indexPath) ? readJson(indexPath) : null;
	if (!index || !Array.isArray(index.repos)) {
		throw new Error("Workspace index is not configured — link/setup the workspace or create its workspace index");
	}
	...
	if (isConcurrentMode(workspaceRoot)) {
		const ctx = resolveExecutionContext({ cwd, host: "cli", createIfMissing: false });
		if (!ctx.taskId) {
			// Zero-task bootstrap parity with the ownership gate: a fresh concurrent
			// workspace allows single-agent writes, so target selection must not
			// require a task that doesn't exist yet.
			if (listTasks(workspaceRoot).length === 0) {
				const bootstrapTarget = {
					...
					selectedBy: "heli-cli-bootstrap",
					reason: `zero-task bootstrap: ${selector}`,
				};
				writeJsonAtomic(targetPath, bootstrapTarget);
				return { needsConfirm: false, target: bootstrapTarget, bootstrap: true };
			}
	...
	switch (subcommand || "show") {
		case "list": {
			...
			if (!result.configured) {
				...
				console.log("Run heli link for a linked project, or configure the workspace index");
				return;
			}
		...
		case "set": {
			...
			if (result.bootstrap) console.log("(concurrent, zero tasks) workspace-level bootstrap target — first task claim becomes authoritative");
