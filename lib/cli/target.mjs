...
export function runTarget(args) {
	...
	const selector = rest.find((arg) => arg !== "--confirm");
	...
	switch (subcommand || "show") {
		...
		case "show": {
			...
			console.log("Usage: heli target list | heli target show | heli target set <repo> [--confirm] | heli target clear");
			return;
		}
		case "set": {
			if (!selector) {
				console.log("Usage: heli target set <repo-name-or-path> [--confirm]");
				return;
			}
			...
			if (result.needsConfirm) {
				...
				console.log(`Re-run with --confirm to proceed: heli target set ${selector} --confirm`);
				return;
			}
		...
		default:
			console.log("Usage: heli target list | show | set <repo> [--confirm] | clear");
	}
}
