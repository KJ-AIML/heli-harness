...
|------|---------|--------|------------------|-------|
| Pi / AXGA | `pi install git:github.com/KJ-AIML/heli-harness@v0.5.16` or `axga install git:github.com/KJ-AIML/heli-harness@v0.5.16` | **supported** | 23 skills + lightweight extension (startup status, `/heli-install`, `/hh-install`, `/hh-status`, `/heli-target`, `/heli-lock`, `/heli-hooks`, `/heli-validate lint`, command-rule guard, safety classifier) | Does not auto-create `.heli-harness/`. Use `/heli-install` or `/hh-install` to set up workspace. Use the v0.5.16 tag after release |
