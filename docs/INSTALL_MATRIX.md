...
|------|---------|--------|------------------|-------|
| Pi | `pi install git:github.com/KJ-AIML/heli-harness@v0.1.1` | **supported** | Pi package: skills + lightweight extension (startup status, `/heli-install`, `/hh-install`, `/hh-status` commands) | Does not auto-create `.heli-harness/`. Use `/heli-install` or `/hh-install` to set up workspace. Local path verified; remote `git:` verified for v0.1.1 |
