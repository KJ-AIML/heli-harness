...
This page is the entry point for **current** Heli architecture. If another document conflicts with this page or the convergence RFC, the current `v0.10.2` implementation and the convergence RFC win.
...
Normal `v0.10.2` use separates distribution from authority:
...
These are current-facing references for `v0.10.2`:
...
1. describe the `v0.10.2` model directly, or
