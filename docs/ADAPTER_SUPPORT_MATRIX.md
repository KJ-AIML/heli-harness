...
| OpenCode | `enforced` | `.heli-harness/adapters/opencode-plugin/`; smoke scripts; `scripts/live-verify-opencode-plugin.mjs` | Copy the plugin tree into `.opencode/plugins/` for automatic project discovery; entry must be `.js`/`.ts` (`.mjs` is not auto-discovered); not a sandbox. |
