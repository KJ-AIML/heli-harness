...
`scripts/live-verify-codex-plugin-install.mjs` drives the real `codex` CLI (isolated `CODEX_HOME`) through `codex plugin marketplace add` and `codex plugin add`, then confirms `codex plugin list` reports the plugin installed and enabled. This proves the install/trust path works against the real Codex CLI. It does not yet prove the PreToolUse hook fires during a real model turn — that requires an actual `codex exec`/interactive turn, which needs Codex usage quota that was unavailable at verification time.
