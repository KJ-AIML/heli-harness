...
| Cursor | `plugin-wired` | `.heli-harness/adapters/cursor-plugin/` marketplace; `node scripts/smoke-cursor-plugin.mjs`; `scripts/verify-adapters.mjs` | Marketplace `.cursor-plugin/marketplace.json` indexes a nested plugin with rules and skills; no runtime guard. |
